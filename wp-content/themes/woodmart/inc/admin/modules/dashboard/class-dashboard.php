...
class Dashboard extends Singleton {
	...
	protected function get_theme_name() {
		$theme_name = esc_html__( 'مدیریو', 'woodmart' );
	...
	public function add_pages_to_admin_bar_menu( $admin_bar ) {
		...
		if ( woodmart_get_opt( 'dummy_import', '1' ) ) {
			$admin_bar->add_node(
				array(
					...
					'title'  => '<i class="xts-i-dummy-content"></i>' . esc_html__( 'دموهای آماده', 'woodmart' ),
					'href'   => admin_url( 'admin.php?page=xts_prebuilt_websites' ),
					'parent' => 'xts_dashboard',
					'meta'   => array(
						'title' => esc_html__( 'دموهای آماده', 'woodmart' ),
					),
				)
			);
		}
		...
		$admin_bar->add_node(
			array(
				'parent' => 'xts_dashboard_external',
				'id'     => 'xts_patcher',
				'title'  => '<i class="xts-i-cog"></i>' . esc_html__( 'پچر', 'woodmart' ) . $patches_count,
	...
	public function add_pages_to_dashboard_menu() {
		...
		add_submenu_page(
			'xts_dashboard',
			esc_html__( 'پچر', 'woodmart' ),
			esc_html__( 'پچر', 'woodmart' ) . $patches_count,
